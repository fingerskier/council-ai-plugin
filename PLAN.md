...
A Claude Code plugin that turns the CLI into a multi-perspective deliberation engine. You **convene** a council of named **seats** (personalities) into a project, then hand it work — quick **jobs**, interactive **meetings**, or autonomous **work** sessions — while a **chair** routes the seats and synthesizes a single answer.
...
The surface is four verbs. `convene` sets up; the other three put the council to work at three escalating levels of autonomy and interactivity.

```
/council convene [template]    # create/recreate .council/ from a template; then edit the files
/council job "<task>"          # chair routes to one or more seats; parallel; fast answer
...
No task runs. This is pure setup — the council only does work via the other three verbs.

### `/council job "<task>"`

The lightweight fan-out. The chair reads the task and **selects one or more seats** whose perspective is relevant (not necessarily all). Selected seats answer **in parallel**, stateless — they don't see each other. The chair folds their takes into a unified answer with dissents preserved.

- Use for: a quick gut-check, a single expert opinion, a breadth scan before deciding.
- No scratchpad, no worktree, no record file by default (it's a conversational answer). Cost: `k` parallel workers + 1 chair, where `k ≤ N`.
...
The **autonomous** mode. One or more councillors **take turns** on the task — **the chair decides who goes next** each turn — with **no user input** during the run. They share the same scratchpad mechanism as `meeting` (`.council/scratch/<id>.md`) so each turn builds on the last.
...
- Use for: a bounded implementation/refactor/research task you want the council to grind on unattended.

### Shared mechanics

- **Scratchpad:** `meeting` and `work` both use `.council/scratch/<session-id>.md` as the shared, append-only conversation seats read before speaking. Deleted or archived to `records/` at session end. `job` does not use one (its workers are parallel and blind).
- **Memory:** all three may read `.council/memory/` for council context; `meeting`/`work` may append durable takeaways on conclusion.
- **Records:** `meeting` and `work` always write a synthesis to `.council/records/`; `job` writes one only if asked.
...
`job` and `meeting` default to **read-only** seats (no worktree) — they produce opinions and prose, not commits. A `meeting` that decides on changes typically hands off to a `work` session.

## 7. Output synthesis

The chair receives the task plus the seats' contributions (parallel takes for `job`, the full scratchpad for `meeting`/`work`) and produces:

1. a **unified recommendation / outcome** (the headline),
2. **dissents** — where seats disagreed and why, preserved not flattened,
3. for `meeting`/`work`, a **record artifact** under `.council/records/` with the reasoning trail, so the deliberation is auditable.
...
- **Interactive (primary):** the orchestrator skill drives Task-tool subagents, building each worker's prompt from `.council/seats/<seat>.md` + injected task (+ scratchpad for `meeting`/`work`). Parallel for `job`, sequential turn-taking for `meeting`/`work`.
- **Chair as router:** in `job` the chair selects which seats answer; in `work` the chair picks who acts each turn and decides termination. The chair is itself a seat (a personality file), so its routing/synthesis voice is tunable.
...
- **Model routing for cost:** run voices on a cheaper model, reserve the strong model for the chair. A wide `job` or a long `work` run can burn far more tokens than a normal session — `work_budget` in `council.yaml` is the explicit guardrail, and `work` honors it as a hard stop.
...
- `/council job` with chair selection + parallel fan-out + synthesis
- the three prebuilt templates + ~12 personality files

**Phase 2 — meeting**
- shared scratchpad under `.council/scratch/`
- sequential round-table with per-round user input
...
2. **`job` seat selection.** Chair picks the relevant subset, or does `job` default to *all* seats unless you name some? Plan assumes chair-selects.
