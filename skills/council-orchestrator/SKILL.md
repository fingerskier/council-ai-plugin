...
6. **Record + memory:** write the synthesis to `.council/records/<id>.md`. Then
   fold durable takeaways into memory **by topic** — for each topic the session
   touched, create or update `.council/memory/<topic>.md` with the decision and
   its *why* (short, not a transcript). Remove the scratchpad (its content now
   lives in the record).
...
8. **Record + memory:** write `.council/records/<id>.md`; fold takeaways into
   memory **by topic** (create/update `.council/memory/<topic>.md` per topic the
   session touched); remove the scratchpad.
